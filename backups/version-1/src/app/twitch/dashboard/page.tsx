    ...
    <div className="min-h-screen bg-black">
      ...
      <main className="container mx-auto px-4 py-12 relative z-10">
        ...
        <div className="mb-12 text-center">
          <h1 className="text-5xl md:text-6xl font-bold mb-4 bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 bg-clip-text text-transparent">
            🎮 لوحة تحكم الستريمر
          </h1>
          <p className="text-xl text-yellow-300/70">مرحباً {user.displayName} 👋</p>
        </div>

        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {/* User Profile Card */}
          <div className="col-span-1 rounded-lg border-2 border-yellow-500/50 p-6" style={{background: 'linear-gradient(135deg, rgba(180, 83, 9, 0.2) 0%, rgba(0, 0, 0, 0.4) 100%)'}}>
            <div className="text-center">
              <img 
              ...
              />
              <h2 className="text-2xl font-bold text-yellow-300 mb-2">{user.displayName}</h2>
              <p className="text-yellow-300/70 mb-4">@{user.login}</p>
              <button
          ...
          {/* Game Control Panel */}
          <div className="col-span-2 rounded-lg border-2 border-yellow-500/50 p-6" style={{background: 'linear-gradient(135deg, rgba(180, 83, 9, 0.2) 0%, rgba(0, 0, 0, 0.4) 100%)'}}>
            <h3 className="text-2xl font-bold text-yellow-400 mb-6">🎮 التحكم باللعبة</h3>

            <div className="mb-6">
              <label className="block text-yellow-300 font-bold mb-2">اختر اللعبة:</label>
              <select
                value={selectedGame}
                onChange={(e) => setSelectedGame(e.target.value)}
                disabled={gameRunning}
                className="w-full bg-slate-800 border-2 border-yellow-500 text-white p-3 rounded-lg disabled:opacity-50"
              >
        ...
        {/* Chat Integration */}
        <div className="rounded-lg border-2 border-yellow-500/50 p-6" style={{background: 'linear-gradient(135deg, rgba(180, 83, 9, 0.2) 0%, rgba(0, 0, 0, 0.4) 100%)'}}>
          <h3 className="text-2xl font-bold text-yellow-400 mb-6">💬 اتصال Twitch Chat</h3>

          <div className="grid md:grid-cols-2 gap-6">
            {/* Chat Status */}
            <div className="p-4 bg-slate-800 rounded-lg border-2 border-yellow-500/30">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-4 h-4 bg-green-500 rounded-full animate-pulse"></div>
                <span className="text-yellow-300 font-bold">متصل بـ Twitch Chat</span>
              </div>
              <p className="text-yellow-300/70 text-sm">جاهز للتفاعل مع المشاهدين في الشات</p>
            </div>

            {/* Features */}
            <div className="p-4 bg-slate-800 rounded-lg border-2 border-yellow-500/30">
              <h4 className="text-yellow-300 font-bold mb-3">المميزات:</h4>
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-6">
          <div className="rounded-lg p-6 bg-gradient-to-br from-yellow-900/30 to-yellow-900/50 border-2 border-yellow-500/50 text-center">
            <div className="text-4xl mb-2">👥</div>
            <p className="text-yellow-300/70">اللاعبون المتصلون</p>
            <p className="text-3xl font-bold text-yellow-400">{playerCount}</p>
          </div>

          <div className="rounded-lg p-6 bg-gradient-to-br from-yellow-900/30 to-yellow-900/50 border-2 border-yellow-500/50 text-center">
            <div className="text-4xl mb-2">🎮</div>
            <p className="text-yellow-300/70">الحالة</p>
            <p className="text-3xl font-bold text-yellow-400">{gameRunning ? '🟢 جاري' : '⚪ متوقفة'}</p>
          </div>
...
export default function TwitchDashboard() {
  return (
    <Suspense fallback={
      <div style={{background: 'linear-gradient(135deg, #000 0%, #1a1a1a 50%, #000 100%)'}} className="min-h-screen flex items-center justify-center">
        <div className="text-center">
